...
- **主区**：命中率仪表盘、当日投资方案、14 日走势、联赛分布、每日预测记录（点开看预测明细/方案历史/复盘）、🎌 **日韩专栏**（每轮 J 联赛/K 联赛各 4 场，id 以「日职」/「韩K」开头，独立累计方向/大小/比分+方案块，并给竞彩方向对照）、🀄 **北单专栏**（id 以「北单」开头的场次单独累计方向/大小/比分+方案块，并给竞彩方向对照）、🦁 **英超专栏**（league 含「英超」单独累计+专家视角）、🚄 **深夜快车专栏**（下半夜至白天 `time < 18:00` 开赛场次单独累计方向/大小/比分，明细带开赛时间列，自动按时间筛选无需额外字段）
...
- `ahPick: '法兰克福-0.5'` + `ahConf: 2` — 亚盘让球观点+信心指数（2026-09-06 起，V3.1 双体系独立：球探定位场每场必推；**2026-09-08 起信心★为 1-5 星**：★1-2=低信心只展示、★3-4=可入 asian7 串关、★5=胆级；未定位场不写该字段）
- `ouPick: '大3.5'` + `ouConf: 3` — 亚盘大小球观点+信心指数（同上，1-5 星；大小盘方案腿=ouConf top 腿）。总览「预测」列分两行：🎫竞彩（方向｜总进球｜比分）/ 🌏亚盘（让球观点★｜大小观点★）。竞彩列方向行 ★ 来自 `confidence`（同样 1-5 星，2026-09-08 起；★5=当日胆，源自 max7 key 腿）
- `ttgSp: '3.60'` — 竞彩总进球 SP（与 overUnder 的 "X球" 预测对应）
- `scoreSp: ['10.0(欧)','SP待开']` — 与 `score` 数组对齐的比分赔率
- `dirTag: '单选'|'双选'|'入拖'|'弃选'` — 方向信心标签（2026-08-28 起），渲染为方向旁徽章；单选>双选>入拖>弃选
- `beidan310: { legs:[{play,match,pick,odds,reason,result}], totalOdds, note, result }` — 北单310 七关顶层块（2026-09-08 起）：pick='3'主胜/'1'平/'0'客胜，match 前三位编号复用当日竞彩场 finalScore 自动判定（leg.result 人工回填 hit/miss 优先）；odds 为竞彩参考 SP（北单 SP 浮动以出票为准）；computeBeidan 自动进北单专栏明细+方案块战绩，总览方案区 market='bd' 归入「🀄 北单方案」组
- `synthesis: '客胜·赢1球'` — 合成观点一句话（2026-08-28 起）：方向×程度，解决"竞彩推客胜、亚盘推主队受让"这类分层表达的方向感矛盾；页面预测列顶部 💡 行展示
